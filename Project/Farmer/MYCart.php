...
<html>
    ...
    <?php
        ...
        if (isset($_POST["btn_checkout"])) {
                ...
                $selC = "select * from tbl_sellerproductbooking where farmer_id='" .$_SESSION["fid"]. "'and spbooking_status='0'";
                $rs = $Conn->query($selC);
                $row=$rs->fetch_assoc();
                 $_SESSION["bid"] = $row["spbooking_id"];
                
                $upQry1 = "update tbl_sellerproductbooking set spbooking_date=curdate(),spbooking_amount='".$amt."',spbooking_status='1' where farmer_id='" .$_SESSION["fid"]. "'";
				$Conn->query($upQry1);
				
				 $upQry1 = "update tbl_sellercart set scart_status='1' where spbooking_id='" .$row["spbooking_id"]. "'";
...
<form method="post">
        <div class="shopping-cart" style="margin-top: 50px">            <?php                
            $sel = "select * from tbl_sellerproductbooking b inner join tbl_sellercart c on c.spbooking_id=b.spbooking_id where b.farmer_id='" .$_SESSION["fid"]. "' and spbooking_status='0' and scart_status=0";
            $res = $Conn->query($sel);
                while ($row=$res->fetch_assoc()) {
                    $selPr = "select * from tbl_sellerproduct where sproduct_id='" .$row["sproduct_id"]. "'";
                    $respr = $Conn->query($selPr);
                    if ($rowpr=$respr->fetch_assoc()) {
                        $selstock = "select sum(sstock_count) as stock from tbl_sellerstock where sproduct_id='".$rowpr["sproduct_id"]."'";
						$selstock1="select sum(scart_quantity) as quantity from tbl_sellercart where sproduct_id='".$rowpr["sproduct_id"]."' and scart_status >'0'";
            ...
            <div class="product">
                <div class="product-image">
                    <img
                        src="../Assets/Files/Photo/<?php echo $rowpr["sproduct_photo"] ?>"
                        />
                </div>
                <div class="product-details">
                    <div class="product-title"><?php echo $rowpr["sproduct_name"] ?></div>
                    <p class="product-description">
                    <?php echo $rowpr["sproduct_details"] ?>
                    </p>
                </div>
                <div class="product-price "	><?php echo $rowpr["sproduct_rate"] ?></div>
                <div class="product-quantity">
                <select id="<?php echo $row["scart_id"]?>" style="width:90px">
                	<?php
					for ($k=1;$k<=($rowst["stock"]-$chk["quantity"]);$k++)
					{
						?>
                        <option <?php if($row["scart_quantity"]==$k){ echo "selected";} ?> value="<?php echo $k ?>"><?php echo $k ?></option>
						<?php
					}
					?>
                </select>
                ...
                <div class="product-removal">
                    <button class="remove-product" value="<?php echo $row["scart_id"] ?>">Remove</button>
                </div>
                <div class="product-line-price">
                    <?php
                        $pr = $rowpr["sproduct_rate"];
                        $qty = $row["scart_quantity"];
                        $tot = (int)$pr * (int)$qty;
                        echo $tot;
                    ?>
                </div>
