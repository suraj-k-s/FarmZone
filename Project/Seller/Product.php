...
if(isset($_POST["btnsave"]))
{
	...
	$insQry ="insert into tbl_sellerproduct(subcategory_id,sproduct_name,sproduct_photo,sproduct_details,sproduct_rate,seller_id) values('".$subcategory_id."','".$name."','".$photo."','".$details."','".$price."','".$_SESSION["sid"]."')";
...
{
    $id=$_GET["id"];	
    $delQry="delete from tbl_sellerproduct where sproduct_id='".$_GET["id"]."'";
...
if(isset($_GET["pid"]))
{
	$selQry="select * from tbl_sellerproduct where sproduct_id='".$_GET["pid"]."'";
	$rel=$Conn->query($selQry);
	if($row=$rel->fetch_assoc())
{
	$pid=$row["sproduct_id"];
	$pname=$row["sproduct_name"];
...
<h1 align="center">Product</h1>
  ...
  <?php
  $i=0;
  $selQry="select * from tbl_sellerproduct p inner join tbl_subcategory s on p.subcategory_id=s.subcategory_id inner join tbl_category c on s.category_id=c.category_id where seller_id='".$_SESSION["sid"]."'";
...
?>
   <tr>
	...
	<td><a href="Product.php?id=<?php echo $row["sproduct_id"];?>">Delete</a></td>
    <td><a href="Stock.php?pid=<?php echo $row["sproduct_id"];?>">stock</a></td>
</tr>
<br>
<?php
	}
