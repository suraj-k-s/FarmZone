...
<html>
    ...
    <?php
        ...
        if (isset($_POST["btn_checkout"])) {
                ...
                $selC = "select * from tbl_farmerproductbooking where user_id='" .$_SESSION["uid"]. "'and fpbooking_status='0'";
                $rs = $Conn->query($selC);
                $row=$rs->fetch_assoc();
                 $_SESSION["bid"] = $row["fpbooking_id"];
                
                $upQry1 = "update tbl_farmerproductbooking set fpbooking_date=curdate(),fpbooking_amount='".$amt."',fpbooking_status='1' where user_id='" .$_SESSION["uid"]. "'";
				$Conn->query($upQry1);
				
				 $upQry1 = "update tbl_farmercart set fcart_status='1' where fpbooking_id='" .$row["fpbooking_id"]. "'";
...
<form method="post">
        <div class="shopping-cart" style="margin-top: 50px">            <?php                
            $sel = "select * from tbl_farmerproductbooking b inner join tbl_farmercart c on c.fpbooking_id=b.fpbooking_id where b.user_id='" .$_SESSION["uid"]. "' and fpbooking_status='0' and fcart_status=0";
            $res = $Conn->query($sel);
                while ($row=$res->fetch_assoc()) {
                    $selPr = "select * from tbl_farmerproduct where fproduct_id='" .$row["fproduct_id"]. "'";
                    $refpr = $Conn->query($selPr);
                    if ($rowpr=$refpr->fetch_assoc()) {
                        $selstock = "select sum(fstock_count) as stock from tbl_farmerstock where fproduct_id='".$rowpr["fproduct_id"]."'";
						$selstock1="select sum(fcart_quantity) as quantity from tbl_farmercart where fproduct_id='".$rowpr["fproduct_id"]."' and fcart_status >'0'";
